namespace ITG_Core {
	...
	public struct RequstSector {
		...
		{
			return "RS: {Coordinate: " + coordinate + " }, width: " + width + ", height: " + height + "}";
		}
	}
	...
	public class Sector<T> where T : struct {
		...
		{
			chunks = new Chunk<T>[width, height];
			this.width = width;
			this.height = height;
			this.coordinate = new Coordinate(coordinate);
			if ( fillup )
		...
		{
			chunks = new Chunk<T>[requstSector.width, requstSector.height];
			width = requstSector.width;
			height = requstSector.height;
			coordinate = requstSector.coordinate;
			if ( fillup )
				FillUp();
		}
		...
		public override string ToString()
		{
			return "S: {Coordinate: " + coordinate + " }, width: " + width + ", height: " + height + "}";
		}
		...
		{
			[MethodImpl(MethodImplOptions.AggressiveInlining)]
			get => chunks[x / Constants.CHUNK_SIZE, y / Constants.CHUNK_SIZE][x % Constants.CHUNK_SIZE, y % Constants.CHUNK_SIZE];
			[MethodImpl(MethodImplOptions.AggressiveInlining)]
			set => chunks[x / Constants.CHUNK_SIZE, y / Constants.CHUNK_SIZE][x % Constants.CHUNK_SIZE, y % Constants.CHUNK_SIZE] = value;
		}
